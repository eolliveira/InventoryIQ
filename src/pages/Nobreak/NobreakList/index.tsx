...
export default function NobreakList() {
  ...
  const handleAddNobreak = () => {
    setFormContextData({ isAdding: true });
    setOpenNobreakForm(true);
  };

  const handleDeleteNobreak = (AssetId: string) => {
    if (selectedAsset == '') {
      Swal.fire({
        title: 'Atenção',
    ...
    <Panel title="Nobreaks">
      <Box display={'flex'} flexWrap={'wrap'} alignItems={'center'} justifyContent={'space-between'} marginBottom={2}>
        ...
        <Stack direction={'row'} spacing={2} flexWrap={'wrap'}>
          ...
          <Button variant="contained" startIcon={<AddCircleOutlineIcon />} color="primary" onClick={handleAddNobreak}>
            <Typography fontSize={14} textTransform={'none'}>
              Novo
            </Typography>
          </Button>
      ...
      {openNobreakForm && <NobreakForm openForm={openNobreakForm} closeForm={() => setOpenNobreakForm(false)} />}
    </Panel>
  );
}
